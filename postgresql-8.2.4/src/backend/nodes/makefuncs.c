		   ...
		   Node *lexpr, Node *rexpr, int location)
{
	...
	/* Changed by Ahmed.M.Aly for SJ purposes since A_Expr is used by
	 * all join types, so a->sj_Type has to be 0 in all joins other than SJ */
	a->sj_Type = 0;
	return a;
}
...
A_Expr *
makeSJ_Expr(A_Expr_Kind kind, const char *name,
		   Node *lexpr, Node *rexpr, int epsilon, int max_diameter)
{
	A_Expr	   *a = makeNode(A_Expr);
	...
	if (epsilon != 0)
		...
		a->sj_Type = 2; // Around
	a->sj_Epsilon = epsilon;
	a->sj_MaxDiam = max_diameter;
	return a;
}
