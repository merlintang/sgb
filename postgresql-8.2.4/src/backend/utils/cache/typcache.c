...
#include "catalog/pg_namespace.h"  /*CHANGED BY YASIN*/
...
{  /*YASIN: WE ADDED SUPPORT FOR FLAG TYPECACHE_MINUS_OPR*/
	...
	if ((flags & (TYPECACHE_EQ_OPR | TYPECACHE_LT_OPR | TYPECACHE_GT_OPR |
		...
		typentry->btree_opc == InvalidOid) /*YASIN we do't add TYPECACHE_MINUS_OPR because this operator is not asociated to btrees*/
	{
	...
	/*CHANGED BY YASIN*/
	if ((flags & TYPECACHE_MINUS_OPR) && typentry->minus_opr == InvalidOid)
	{
		/*Search directly */
		HeapTuple	opertup;
		//NameData opname;
		...
		opertup = SearchSysCache(OPERNAMENSP, /*name of cache*/
							...
							ObjectIdGetDatum(type_id),/*letf op type*/
							ObjectIdGetDatum(type_id),/*right op type*/
							ObjectIdGetDatum(PG_CATALOG_NAMESPACE)); /*id of pg_catalog namespace*/
		
		if (HeapTupleIsValid(opertup))
		{
			typentry->minus_opr = HeapTupleGetOid(opertup);
			ReleaseSysCache(opertup);
		}
		else /*If it is not valid*/
			typentry->minus_opr = InvalidOid;
	}
