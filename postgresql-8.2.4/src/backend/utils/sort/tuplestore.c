...
tuplestore_puttuple_common(Tuplestorestate *state, void *tuple)
{
	switch (state->status)
	{
		...
		case TSS_READFILE:
			...
			break;
		default:
			elog(ERROR, "invalid tuplestore state put common");
			break;
	}
}
					...
					bool *should_free)
{
	...
	switch (state->status)
	{
		...
		default:
			...
			elog(ERROR, "invalid tuplestore get tuple");
...
tuplestore_rescan(Tuplestorestate *state)
{
	switch (state->status)
	{
		...
		default:
			//elog(ERROR, "invalid tuplestore state");
			elog(ERROR, "invalid tuple store rescan");
			break;
	}
}
...
tuplestore_restorepos(Tuplestorestate *state)
{
	switch (state->status)
	{
		...
		case TSS_READFILE:
			...
			break;
		default:
			elog(ERROR, "invalid tuplestore state tuplestore_restorepos");
			break;
	}
}
