...
int MySearchCallback(int id, void *args)
{
 /* Note: -1 to make up for the +1 when data was inserted */
	 if(iscontain_M)
	    {
		 	 containArray[index_contain]=id;
		 	 index_contain++;
	    	 //printf("contain this MBR id is %d \n", id);
	    }else
	    {
	    	overlapArray[index_overlap]=id;
	    	index_overlap++;
	    	//printf("overlap but not contain id is %d \n", id);
	    }
...
extern
void initilListBegin()
{
    index_contain=0;
    index_overlap=0;
    memset(containArray,0,sizeof(int)*sizeofgroup);
    memset(overlapArray,0,sizeof(int)*sizeofgroup);
}
...
void initilList()
{
	...
	//index_overlap=0;
       ...
       if(index_contain>0)
        memset(containArray,0,sizeof(int)*index_contain);
       
        if(index_overlap>0)
	memset(overlapArray,0,sizeof(int)*index_overlap);
...
static void _RTreePrintPart(HRTREEROOT root, RTREEPARTITION *p)
{
 ...
 fprintf (stdout, "/npartition:/n");
 for (i=0; i<p->total; i++)
 {
  fprintf (stdout, "%3d/t", i);
 }
 fprintf (stdout, "/n");
 for (i=0; i<p->total; i++)
 {
 ...
 for (i=0; i<p->total; i++)
 {
  fprintf (stdout, "%3d/t", p->partition[i]);
 }
 fprintf (stdout, "/n");

 fprintf (stdout, "count[0] = %d  area = %f/n", p->count[0], p->area[0]);
 fprintf (stdout, "count[1] = %d  area = %f/n", p->count[1], p->area[1]);
 if (p->area[0] + p->area[1] > 0)
 {
  fprintf (stdout, "total area = %f  effectiveness = %3.2f/n",
   p->area[0] + p->area[1], (float)root->CoverSplitArea / (p->area[0] + p->area[1]));
 }
 fprintf (stdout, "cover[0]:/n");
 RTreePrintRect(&p->cover[0], 0);

 fprintf (stdout, "cover[1]:/n");
...
{
 return (RTREELISTNODE *) palloc(sizeof(RTREELISTNODE));
}
 ...
 */
static int _RTreeSearchRect( RTREENODE *node, RTREEMBR *mbr, PFNRTreeSearchCallback pfnSHCB, void* pfnParam, bool iscontain, bool overlapisrandom)
{
 /* Fix not yet tested. */
 int hitCount = 0;
 int i;
 ...
 if (node->level > 0) /* this is an internal node in the tree */
 {
  for (i=0; i<NODECARD; i++){
   if (node->branch[i].child && RTreeOverlap(mbr, &node->branch[i].mbr))
   {
    hitCount += _RTreeSearchRect(node->branch[i].child, mbr, pfnSHCB, pfnParam, iscontain,overlapisrandom);

    if(overlapisrandom==true&&hitCount>3)
    {
    	return hitCount;
    }
...
#pragma warning( disable : 4311 )
  for (i=0; i<LEAFCARD; i++)
  {
	  if(iscontain==true) //only find the RMB that contain the input
	  {
		  if (node->branch[i].child && RTreeOverlap(mbr, &node->branch[i].mbr)&& RTreeContained(mbr,&node->branch[i].mbr))
		   {
		    hitCount++;
		   //find the RMB overlap with this input rectangle
	  ...
	  }else //only find the RMB that is overlap but not contain
	  {
		  if (node->branch[i].child && RTreeOverlap(mbr, &node->branch[i].mbr)&&!RTreeContained(mbr,&node->branch[i].mbr))
		   {
		    hitCount++;
...
/**
 * Search in an index tree or subtree for all data rectangles that was contained by the argument rectangle.
 * Return the number of qualifying data rects.
 */
static int _RTreeSearchRect_contain( RTREENODE *node, RTREEMBR *mbr, PFNRTreeSearchCallback pfnSHCB, void* pfnParam)
{
 /* Fix not yet tested. */
 int hitCount = 0;
 int i;
 ...
 if (node->level > 0) /* this is an internal node in the tree */
 {
  for (i=0; i<NODECARD; i++){
   if (node->branch[i].child && RTreeOverlap(mbr, &node->branch[i].mbr))
   {
    hitCount += _RTreeSearchRect_contain(node->branch[i].child, mbr, pfnSHCB, pfnParam);
...
#pragma warning( disable : 4311 )
  ...
  {
	  {
		  if (node->branch[i].child &&RTreeContained(&node->branch[i].mbr,mbr))
		   {
		    hitCount++;
		   //find the RMB overlap with this input rectangle
		     /* call the user-provided callback and return if callback wants to terminate search early */
...
{
 RTREENODE *node = (RTREENODE*) palloc(sizeof(RTREENODE));
 assert(node);
 RTreeInitNode(node);
 return node;
}
...
/*
#pragma warning( disable : 4311 )
 fprintf (stdout, "  level=%d  count=%d  address=%o/n", node->level, node->count, (unsigned int) node);
 ...
 for (i=0; i<node->count; i++)
 {
  if(node->level == 0) {
   /* _RTreeTabIn(depth); */
/*
   fprintf (stdout, "/t%ld: data = %ld/n", i, (unsigned int)node->branch[i].child);
  }
  else {
   _RTreeTabIn(depth);
   fprintf (stdout, "branch %d/n", i);
...
{
 RTREEROOT * root = (RTREEROOT*) palloc(sizeof(RTREEROOT));
 assert(root);
...
* Return the number of qualifying data rects.
*/
int RTreeSearch(HRTREEROOT root, RTREEMBR *mbr, void* pfnSearchCallbackParam, bool iscontain, bool overlapisrandom)
{
 return  _RTreeSearchRect(root->root_node, mbr, root->pfnCallback, pfnSearchCallbackParam, iscontain, overlapisrandom);
}

/**
* Search in an index tree for all data rectangles that was contained by the input argument mbr
* Return the number of qualifying data rects.
*/
 int RTreeSearch_Contain(HRTREEROOT root, RTREEMBR *mbr, void* pfnSearchCallbackParam)
{

	 return  _RTreeSearchRect_contain(root->root_node, mbr, root->pfnCallback, pfnSearchCallbackParam);
}
