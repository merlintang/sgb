...
ExecInitValuesScan(ValuesScan *node, EState *estate, int eflags)
{
	...
	/*CHANGED BY YASIN*/
	if (eflags & EXEC_FLAG_INAROUND) scanstate->ss.ps.ps_InAround = true;
...
#define VALUESSCAN_NSLOTS 2
	...
	/*CHANGED BY YASIN*/
	if (scanstate->ss.ps.ps_InAround) /*In case we use around, we use the around range table*/
		/*rte = rt_fetch(node->scan.scanrelid, estate->es_range_table_around); */
