...
InitPlan(QueryDesc *queryDesc, int eflags)
{
	...
	/*CHANGED BY YASIN*/
	/*We get the group around range table if around is being used*/
	/*COMMENTED BY YASIN FOR TESTING. IT WILL BE REMOVED PERMANENTEMENTE IN FINAL VERSION
	if (parseTree->hasAround)
	{

		index = 0;
		foreach(l, parseTree->groupClause)
		{
			if (index==0){
				GroupClause *gc = (GroupClause *) lfirst(l);
				Query *qaround = (Query *)gc->aroundSubquery;
				estate->es_range_table_around = qaround->rtable;
				break; 
			}
		}
	}*/
	...
	planstate = ExecInitNode(plan, estate, eflags);/*YASIN in eflags we also have the knowledge of if the node belongs to the around branch or not*/	
...
ExecInsert(TupleTableSlot *slot,
		   ItemPointer tupleid, //not reference
		   TupleTableSlot *planSlot,
		   DestReceiver *dest,
		   EState *estate)
{
