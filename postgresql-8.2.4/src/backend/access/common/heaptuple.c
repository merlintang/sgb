...
heap_modify_tuple(HeapTuple tuple,
				  TupleDesc tupleDesc,
				  Datum *replValues, /*array*/
				  bool *replIsnull, /*array*/
				  bool *doReplace) /*array*/
{
	int			numberOfAttributes = tupleDesc->natts;
	int			attoff;
	Datum	   *values;
	 ...
	 */
	values = (Datum *) palloc(numberOfAttributes * sizeof(Datum)); /*array*/
	isnull = (bool *) palloc(numberOfAttributes * sizeof(bool)); /*array*/

	heap_deform_tuple(tuple, tupleDesc, values, isnull);/*BUILD THE ARRAYS values,isnull from the ORIGINAL tuple*/

	for (attoff = 0; attoff < numberOfAttributes; attoff++)
	{
		if (doReplace[attoff])/*If we should replace*/
		{
	 ...
	 */
	newTuple = heap_form_tuple(tupleDesc, values, isnull);/*form a NEW tuple, uses palloc*/
...
slot_getattr(TupleTableSlot *slot, int attnum, bool *isnull)
{
	 ...
	 */
	slot_deform_tuple(slot, attnum); /*build arrays until attnum from tuple. it uses arrays of slot!*/
