...
static GroupClause *
make_group_clause(TargetEntry *tle, List *targetlist, Oid sortop, Node *aroundQuery, int sgb_MaxGroupDiam, int sgb_MaxElemSep, int sgb_DistanceAll, int sgb_overlap,int sgb_DistanceAny, int sgb_metric ) /*CHANGED BY YASIN and RUBY added sgb_DistanceAll*/
{
	GroupClause *result;
	...
	result->aroundRefPointsSubquery = aroundQuery; /*CHANGED BY YASIN*/
	result->max_group_diam = sgb_MaxGroupDiam; /*CHANGED BY YASIN*/ /*will be -1 is not used*/
	result->max_elem_sep = sgb_MaxElemSep; /*CHANGED BY YASIN*/ /*will be -1 is not used*/
	result->distanceall = sgb_DistanceAll; /*CHANGED BY RUBY*/ /*will be -1 is not used*/
    result->overlapValue = sgb_overlap;
    result->distanceanyValue=sgb_DistanceAny;
    result->metricValue=sgb_metric;
...
transformGroupClause(ParseState *pstate, List *grouplist,
					 ...
					 Node *aroundSelectStmt, bool *hasAroundSGB, bool *hasUnsupervisedSGB,
					 int sgb_MaxGroupDiam, int sgb_MaxElemSep,bool *hasDistAll, int sgb_distanceAll, int sgb_overlap, int sgb_distanceAny,
					 bool *hasDistAny,  int sgb_metric)
{
	//printf("overlap group clause ###### %d \n",sgb_overlap );
	List	   *result = NIL;
	List	   *tle_list = NIL;
	ListCell   *l;
	int			index; /*To control who receives the around subselect*/ /*CHANGED BY YASIN*/
	...
	/*CHANGED BY YASIN*/ /*Currently we support either SGB_Around or SGB_Unsupervised*/
	index=0;
	if (aroundSelectStmt != NULL) /*For now, we assume that we only receive 0 or 1 around subselects*/
	{
		*hasAroundSGB=true;
		*hasUnsupervisedSGB=false;
		*hasDistAll = false;
	}


	else if (sgb_distanceAll != -1 )
	{
		*hasDistAll = true;
		*hasAroundSGB=false;
		*hasUnsupervisedSGB=false;
	...
	else /*It is not Around*/
	{
		*hasAroundSGB=false;
		*hasDistAll = false;

		if ( (sgb_MaxGroupDiam!=-1) || (sgb_MaxElemSep!=-1) )
			*hasUnsupervisedSGB=true;
		else
			*hasUnsupervisedSGB=false;
	}

	/*THIS CODE GENERATES qry->groupClause (2 PARTS)
	...
	foreach(l, sortClause)
	{
		...
		foreach(tl, tle_list)
		{
			...
			if (sc->tleSortGroupRef == tle->ressortgroupref)
			{
				...
				/*CHANGED BY YASIN and RUBY added sgb_distanceAll*/
				index++;
				if ((index == 1) && (*hasAroundSGB == true)) /*For now, we use around only in the first column of group by*/ /*CHANGED BY YASIN*/
				{
						List	   *qtrees;
						Query	   *qtree;

						qtrees = parse_sub_analyze(aroundSelectStmt, pstate); /*This will call do_parse_analize for the sub query*/
						if (list_length(qtrees) != 1)
							elog(ERROR, "bad query in sub-select");
						qtree = (Query *) linitial(qtrees);
						if (qtree->commandType != CMD_SELECT || qtree->into != NULL)
							elog(ERROR, "bad query in sub-select");

						/* Use the sort clause's sorting operator */
						gc = make_group_clause(tle, *targetlist, sc->sortop, (Node *) qtree, sgb_MaxGroupDiam, sgb_MaxElemSep,sgb_distanceAll,sgb_overlap, sgb_distanceAny, sgb_metric);

				}
				else /*Original code*/
				{
					/* Use the sort clause's sorting operator */
					gc = make_group_clause(tle, *targetlist, sc->sortop, NULL, sgb_MaxGroupDiam, sgb_MaxElemSep, sgb_distanceAll,sgb_overlap, sgb_distanceAny,sgb_metric);/*max_variables are -1 if not used*/
				}
	...
	foreach(l, tle_list)
	{
		...
		sort_op = ordering_oper_opid(exprType((Node *) tle->expr)); /*FIND OPERATOR '<'*/
		...
		if ((index == 1) && (*hasAroundSGB == true)) /*For now, we use around only in the first column of group by*/ /*CHANGED BY YASIN*/
		{
			...
			qtrees = parse_sub_analyze(aroundSelectStmt, pstate);
			if (list_length(qtrees) != 1)
				elog(ERROR, "bad query in sub-select");
			qtree = (Query *) linitial(qtrees);
			if (qtree->commandType != CMD_SELECT || qtree->into != NULL)
			...
			/* Use the sort clause's sorting operator */
			gc = make_group_clause(tle, *targetlist, sort_op, (Node *) qtree, sgb_MaxGroupDiam, sgb_MaxElemSep,  sgb_distanceAll, sgb_overlap, sgb_distanceAny, sgb_metric);
		}
		else
		{
			/* Use the sort clause's sorting operator */
			gc = make_group_clause(tle, *targetlist, sort_op, NULL, sgb_MaxGroupDiam, sgb_MaxElemSep,  sgb_distanceAll,sgb_overlap, sgb_distanceAny, sgb_metric);/*max_variables are -1 if not used*/
		}
