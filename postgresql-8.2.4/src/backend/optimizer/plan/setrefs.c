...
/*CHANGED BY YASIN*/
static void adjust_plan_varnos_around(Plan *plan, int rtoffset);
...
/*CHANGED BY YASIN*/
static Plan *set_aroundquery_references(Plan *plan, List *rtable);
...
set_plan_references(Plan *plan, List *rtable)
{
	...
	if (plan == NULL)
		return NULL;
	/*printf("PLANNER: plan at beginning of set_plan_references\n");pprint(plan);
	printf("PLANNER: range table at beginning of set_plan_references\n");pprint(rtable);*/
	...
	switch (nodeTag(plan))
	{
		...
		case T_Agg:
		case T_Group:
			set_uppernode_references(plan, (Index) 0);/*makes agg.targetlist point to agg.lefttree.targetlist elements*/
			fix_expr_references(plan, (Node *) plan->targetlist);/*finds operators info and calls set_plan_references for subqueries*/
			fix_expr_references(plan, (Node *) plan->qual);/*finds operators info and calls set_plan_references for subqueries*/
			break;
		case T_Result:
	...
	/*CHANGED BY YASIN*/
	if (nodeTag(plan) == T_Agg || nodeTag(plan) == T_Group){/*We are processing AGG or GROUP*/
		/*standard processing of the right tree*/
		plan->righttree = set_plan_references(plan->righttree, rtable);

		/*Move rangetable of right child to main rangetable and update references*/
		if (plan->righttree != NULL)/*If it is around*/
			plan->righttree = set_aroundquery_references(plan, rtable);
...
set_subqueryscan_references(SubqueryScan *plan, List *rtable)
{
	...
	rte = rt_fetch(plan->scan.scanrelid, rtable);/*plan->scan.scanrelid in the index of the corresponding entry in rtable*/
	Assert(rte->rtekind == RTE_SUBQUERY);
	/*printf("PLANNER: PLAN BEFORE llamada recursiva a set_plan_references\n");pprint(plan);
	printf("PLANNER: main query range table BEFORE llamada recursiva a set_plan_references\n");pprint(rtable);*/

	plan->subplan = set_plan_references(plan->subplan, /*This is the only chind of this node(plan)*/
						rte->subquery->rtable);/*points to query.rtable[0].subquery.rtable in our example*/
	/*printf("PLANNER: PLAN after llamada recursiva a set_plan_references\n");pprint(plan);
	printf("PLANNER: main query range table after llamada recursiva a set_plan_references\n");pprint(rtable);*/
	...
	/*CHANGED BY YASIN ONLY FOR TESTING!!!!!!!!!!!*/
	if (trivial_subqueryscan(plan))/*If subqueryscan should be removed. Our case*/
	//if (false)/*If subqueryscan should be removed. Our case*/
	{
		/*
		 ...
		 */
		int			rtoffset = list_length(rtable);/*main rangetable lenght*/
		List	   *sub_rtable;
		ListCell   *lp,
				   *lc;

		/*PROCESS SUBQUERY RANGE TABLE*/
		sub_rtable = copyObject(rte->subquery->rtable);/*This is the range table of the subquery*/
		range_table_walker(sub_rtable,
						   adjust_expr_varnos_walker,/*Function to execute in each node*/
						   (void *) &rtoffset,
						   QTW_IGNORE_RT_SUBQUERIES);/*adds rtoffset in every Var of sub_rtable*/
		rtable = list_concat(rtable, sub_rtable);/*rtable = rtable || subquery.rtable*/

		/*PROCESS SUBQUERY PLAN TREE*/
		/*
		 ...
		 * varnos twice
		 */
		result = copyObject(plan->subplan);/*Initializes result with copy of subplan*/
		adjust_plan_varnos(result, rtoffset);/*Offsets varnos and other rangetable indexes in subplan TREE*/
											 /*Note that this function modifies result directly*/
		result->initPlan = list_concat(plan->scan.plan.initPlan,/*initPlan of main(current) node*/
									   result->initPlan); /*initPlan of subquery*/
		/*printf("PLANNER: PLAN 2\n");pprint(plan);
		printf("PLANNER: main query range 2\n");pprint(rtable);*/
...
/*CHANGED BY YASIN*/
/*
 * set_aroundquery_references
 *		copies range table of aroun queries to main query and adjust var references
 * Input: plan points to the AGG or GROUP node no to its right child directly
 */
static Plan *
set_aroundquery_references(Plan *plan, List *rtable)
{
	Plan	   *result;
	List	   *sub_rtable;
	int			rtoffset;
	result = NULL;
	sub_rtable = NULL;
	rtoffset = list_length(rtable);/*main rangetable lenght*/

	/*PROCESS AROUND QUERY RANGE TABLE*/
	if (nodeTag(plan) == T_Agg)
		sub_rtable = copyObject(((Agg *)plan)->subquery_around_rtable);/*This is the range table of the around query*/
	else if (nodeTag(plan) == T_Group)
		sub_rtable = copyObject(((Group *)plan)->subquery_around_rtable);/*This is the range table of the around query*/
	range_table_walker(sub_rtable,
					   adjust_expr_varnos_walker,/*Function to execute in each node*/
					   (void *) &rtoffset,
					   QTW_IGNORE_RT_SUBQUERIES);/*adds rtoffset in every Var of sub_rtable*/
	...
	/*PROCESS AROUND QUERY PLAN TREE*/
	/*
	 * we have to copy the subplan to make sure there are no duplicately
	 * linked nodes in it, else adjust_plan_varnos might increment some
	 * varnos twice
	 */
	if (nodeTag(plan) == T_Agg)
		result = copyObject(((Agg *)plan)->plan.righttree);/*Initializes result with copy of around query plan*/
	else if (nodeTag(plan) == T_Group)
		result = copyObject(((Group *)plan)->plan.righttree);/*Initializes result with copy of around query plan*/
	adjust_plan_varnos_around(result, rtoffset);/*Offsets varnos and other rangetable indexes in subplan TREE*/
										 /*Note that this function modifies result directly*/
...
/*
 * adjust_plan_varnos_around
 *		Offset varnos and other rangetable indexes in a plan tree by rtoffset.
 * 		Does not process right child of AGG or GROUP
 */
static void
adjust_plan_varnos_around(Plan *plan, int rtoffset)
{
	ListCell   *l;
	...
	adjust_plan_varnos(plan->lefttree, rtoffset);
	/*CHANGED BY YASIN*/
	if ((nodeTag(plan) != T_Agg) && (nodeTag(plan) != T_Group))/*process right plan only if node is not AGG or GROUP*/
		adjust_plan_varnos(plan->righttree, rtoffset);
}
