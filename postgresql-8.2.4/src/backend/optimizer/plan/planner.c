...
/*	printf("PLANNER: PLAN BEFORE set_plan_references\n");pprint(result_plan);
	printf("PLANNER: main query range table BEFORE set_plan_references\n");pprint(parse->rtable);*/

	result_plan = set_plan_references(result_plan, parse->rtable);
/*	printf("PLANNER: PLAN AFTER set_plan_references\n");pprint(result_plan);
	printf("PLANNER: main query range table AFTER set_plan_references\n");pprint(parse->rtable);*/
				 ...
				 List **subquery_pathkeys)
{
	...
	/*
	printf("ENTRA A subquery_planner CON PARSE::\n");	pprint (parse);
	*/
	 ...
	 */
	/*printf("PLAN before SS_finalize_plan\n"); pprint(plan);*/
	if (PlannerPlanId != saved_planid || PlannerQueryLevel > 1)
		SS_finalize_plan(plan, parse->rtable);
	/*printf("plan after SS_finalize_plan\n"); pprint(plan);*/
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	else
	{
		...
		Plan	   *planAround; /*CHANGED BY YASIN*/
		double		tuple_fraction_around;/*CHANGED BY YASIN*/
		ListCell   *l;/*CHANGED BY YASIN*/
	    int			indexSGB;/*CHANGED BY YASIN*/
		Query	   *subquery_around;/*CHANGED BY YASIN*/
	    List *subquery_around_rtable = NULL;/*CHANGED BY YASIN*/
	    int 		max_group_diam = -1; /*CHANGED BY YASIN*/
	    int 		max_elem_sep = -1; /*CHANGED BY YASIN*/
	    int			distanceall = -1; /*CHANGED BY RUBY*/
	    int			overlap = -1;
	    int 		distanceany=-1;
	    int 	 	metric=-1;
		...
		/*printf("resultado de make_subplanTargetList \n");pprint(sub_tlist);*/
		 ...
		 */
		if (parse->hasAggs) /*HERE IT COUNTS THE AGGREGATIONS USED IN TARGET LIST AND IN HAVING QUAL*/
		{
		...
		/*printf("cheapest_path of query_planner\n");  pprint(cheapest_path);
		printf("sorted_path of query_planner\n");  pprint(sorted_path);	*/


		/* CHANGED BY YASIN
		 * We load parameters for SGB and generate the plan for the select of GROUP AROUND
		 * We use the first group by column
		 */
		subquery_around = NULL;
		planAround = NULL;
		if (parse->hasAroundSGB){//There is at least one group around
			tuple_fraction_around=  0.0;	/* default behavior, we expect to use all the around points */

			/*Get the query of the first GroupClause (1st group-by column)*/
			indexSGB=0;
			foreach(l, parse->groupClause)/*We analize each node of groupClause, this code can be used in the future to support n columns*/
			{
				GroupClause *gc = (GroupClause *) lfirst(l);
				indexSGB++;
				if (indexSGB==1){/*For now, we use only the first column*/
					/*We get a pointer to the subquery*/
					subquery_around =(Query *) gc->aroundRefPointsSubquery;
				    max_group_diam = gc->max_group_diam;
					max_elem_sep = gc->max_elem_sep;

					/*We get also a pointer to the subquery's rtable*/
					subquery_around_rtable = subquery_around->rtable;
				}
				else
					break;
			}
			/*We generate the plan for the around query*/
			planAround = subquery_planner(subquery_around, tuple_fraction_around, NULL);
			/*printf("planAround\n");  pprint(planAround);*/

			/*We UPDATE the number of groups since we will use group-around*/
			/*Now dNumGroups = the number of rows of the single select around.
			 * If several cols are used, we need to stimate the final number of groups using the number of rows
			 * of the select of each col. We can use the same euristic used in group by:
			 * num-groups=(rows in col1 * rows in col2*...)/10 */
		    dNumGroups = (double) planAround->plan_rows;
		}
		else if (parse->hasUnsupervisedSGB){//There is at least one unsupervised SGB
			/*Get the parameters of the first GroupClause (1st group-by column)*/
			indexSGB=0;
			foreach(l, parse->groupClause)/*We analize each node of groupClause, this code can be used in the future to support n columns*/
			{
				GroupClause *gc = (GroupClause *) lfirst(l);
		...
		else if (parse->hasDistAll)
		{
			...
			foreach(l, parse->groupClause)/*We analize each node of groupClause, this code can be used in the future to support n columns*/
			{
				...
				if (indexSGB==1){/*For now, we use only the first column*/
					/*We get a pointer to the subquery*/
					distanceall = gc->distanceall;
					overlap = gc->overlapValue; /*CHANGED BY RUBY*/
					metric=gc->metricValue;
		...
		/*CHANGED BY RUBY*/
			else if (parse->hasDistAny)
			{
				...
				foreach(l, parse->groupClause)/*We analize each node of groupClause, this code can be used in the future to support n columns*/
				{
					GroupClause *gc = (GroupClause *) lfirst(l);
					indexSGB++;
					//printf("942 planner !!!! %d \n",gc->distanceanyValue);
					if (indexSGB==1){/*For now, we use only the first column*/
						/*We get a pointer to the subquery*/
						distanceany = gc->distanceanyValue;
						metric=gc->metricValue;
						//overlap = gc->overlapValue; /*CHANGED BY RUBY*/
		...
		if (parse->groupClause)
		{   /*CHANGED BY YASIN*/
			if (parse->hasAroundSGB || parse->hasUnsupervisedSGB)/*Query uses SGB*/{
				/*MAIN CONFIGURATION NOTES. (See also executor/nodeAgg.c)*/

				/* We detect the use of Group Around if parse->hasAroundSGB or parse->hasUnsupervisedSGB is TRUE
				 *
				 * The desition between hash around-grouping Vs sort around-grouping is determined by the
				 * configuration parameter enable_hashagg (postgresql.conf).
				 * If this parameter is TRUE(default), then hash around-grouping is used ALWAYS
				 *    Note: - Currently hash around-grouping supports SGB (Around or Unsupervised) on 1 col, and GB on several cols
				 *          - Multiple CA cols are supported using several aggregation nodes
				 *
				 * If this parameter is FALSE, then sort around-grouping is used ALWAYS
				 *    Note: - Currently sort around-grouping supports GAROUND on 1 col, and 0 GB cols
				 *          - This option should not be selected for queries that require support multiple aggregation cols
				 *          - This option does not support UnsupervisedSGB
				 */

				if (enable_hashagg) /*IF parameter enable_hashagg (postgresql.conf) is TRUE*/
					use_hashed_grouping = true;
				else /*IF parameter enable_hashagg (postgresql.conf) is FALSE*/
					use_hashed_grouping = false;
			}
			/*CHANGED BY RUBY*/
			else if (parse->hasDistAll || parse->hasDistAny)
			{
				//use_hashed_grouping = false;
				use_hashed_grouping = false;
			}
			else /*Original code. (If around is not used)*/
				use_hashed_grouping =
					choose_hashed_grouping(root, tuple_fraction,
										   cheapest_path, sorted_path,
										   dNumGroups, &agg_counts); /*Does not modify any of its parameters*/

			/* Original code continues. Also convert # groups to long int --- but 'ware overflow! */
			numGroups = (long) Min(dNumGroups, (double) LONG_MAX);
		}

		/*
		 * Select the best path FOR LEFT PATH.  If we are doing hashed grouping, we will
		 * always read all the input tuples, so use the cheapest-total path.
		 * Otherwise, trust query_planner's decision about which to use.
		 */
		if (use_hashed_grouping || !sorted_path)/*THIS SORTED PATH IS SORTED BY ITSELF: WITHOUT EXPLICIT SORTING*/
			best_path = cheapest_path; /*Notice that this option is selected when around is used*/
		else
		 ...
		 */
		/*CHANGED BY YASIN*/
		if (parse->hasAroundSGB || parse->hasUnsupervisedSGB || parse->hasDistAll )/*We do not do this optimization if SGB is used*/
			result_plan = NULL;
		else /*Original code*/
		...
		else
		{
			...
			/*printf("result_plan for best_path\n"); pprint(result_plan);*/
			...
			/*CHANGED BY YASIN*/
			if (use_hashed_grouping) /*By default hash groping is used for SGB grouping*/
			{
				/* Hashed aggregate plan*/
				if (parse->hasAroundSGB || parse->hasUnsupervisedSGB){/*SGB*/
					/*Add sort (on right-hand side) only on the SGB col (first group-by col)*/

					/*COMMENTED BY YASIN ONLY FOR TESTING WITHOUT EXTRA SORT*/
					if (!pathkeys_contained_in_sgb(group_pathkeys, /*If it is already sorted we don't need to sort here*/
											   current_pathkeys)) /*this ONLY considers the first col (the one using around)*/
					{
						result_plan = (Plan *)
							make_sort_from_groupcols_sgb(root, /*this sorts only for the first col (the one using SGB)*/
													 parse->groupClause,
													 groupColIdx,
													 result_plan);
					}
					...
					/*add AGG node*/
					result_plan = (Plan *) make_agg_SGB(root,
												tlist,
												...
												result_plan,
												planAround,
												subquery_around_rtable,
												max_group_diam,
												max_elem_sep);

					/*printf("result_plan: Output of make_agg_around\n");	pprint(result_plan);*/
				}
				else{/* Original code. No sort needed in (non SGB) hash grouping*/
			...
			/*if hash grouping is not used. Sorted grouping*/
			/*Yas IMPORTANT NOTE: Group-Around uses hashing by default. But since we can use sorting
			 *for GA 1 col and 0 regular GB we leave the code that handles the sorting case*/

			else if (parse->hasAggs) /*HAS AGGREGATIONS*/
			{/*This is sort-GA CASE 1. Queries that use sort-group around can enter in this case*/
				/* Plain aggregate plan --- sort if needed */
				AggStrategy aggstrategy;

				if (parse->groupClause) /*HAS AGGREGATIONS AND GROUPS*/
				{/*Queries that use around can enter in this case*/
				...
				else/*HAS ONLY AGGREGATIONS,NO GROUPS: Output=1 row, this case do not make use of AROUND*/
				{ /*Note: If query use around cannot be this case*/
				...
				/*CHANGED BY YASIN*/
				if (parse->hasAroundSGB){ /*Currently supports SGB_around*/
				result_plan = (Plan *) make_agg_SGB(root,
												tlist,
												(List *) parse->havingQual,
												aggstrategy,
				...
				{
					result_plan = (Plan *) make_agg_SGB_DIST(root,
																	tlist,
																	(List *) parse->havingQual,
																	AGG_HASHED,
																	//AGG_PLAIN,
																	...
																	numGroups,
																	agg_counts.numAggs,
																	result_plan, distanceall, overlap,distanceany,metric);
							...
							{
					//printf("distanceany 1231 planner %d \n",distanceany );
						result_plan = (Plan *) make_agg_SGB_DIST(root,
																		tlist,
																		(List *) parse->havingQual,
																		AGG_HASHED,
			...
			else if (parse->groupClause)/*HAS ONLY GROUPS, NO AGGREGATIONS*/
			{/*This is sort-GA CASE 2. Queries that use sort-group around can enter in this case*/
				/*
				...
				/*CHANGED BY YASIN*/
				if (parse->hasAroundSGB){/*Currently supports SGB_Around*/
					result_plan = (Plan *) make_group_around(root,
													  tlist,
													  (List *) parse->havingQual,
													  numGroupCols,
													  groupColIdx,
					   ...
					   bool *need_tlist_eval)
{
	...
	*groupColIdx = NULL;
	...
	/* YAS: upper- level IS USED WHEN THE CURRENT QUERY REFERENCES OUTHER RELATIONS
