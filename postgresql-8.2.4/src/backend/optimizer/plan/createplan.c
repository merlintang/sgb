					  ...
					  Plan *inner_plan)
{
	...
	/* Modified by Ahmed.M.Aly */
	join_plan->SJ_Type = root->parse->jointree->SJ_Type;
	join_plan->epsilon = root->parse->jointree->epsilon;
	join_plan->max_diameter = root->parse->jointree->max_diameter;
 ...
 * 'grpColIdx' gives the column numbers to use. THE INDEX OF THE COLUMNS BEING USED IN GROUP BY EXPRESION
 *
...
/*CHANGED BY YASIN*/
/*Similar to make_sort_from_groupcols but only considers the first column of group-by list*/
Sort *
make_sort_from_groupcols_sgb(PlannerInfo *root,
						 List *groupcls,
						 AttrNumber *grpColIdx,
						 Plan *lefttree)
{
	...
	TargetEntry *tle;
	 ...
	 /*We will need at most 1 sort columns*/
	numsortkeys = 1;
	sortColIdx = (AttrNumber *) palloc(numsortkeys * sizeof(AttrNumber));
	sortOperators = (Oid *) palloc(numsortkeys * sizeof(Oid));
	numsortkeys = 0;

	l = list_head(groupcls);

	grpcl = (GroupClause *) lfirst(l);
	tle = get_tle_by_resno(sub_tlist, grpColIdx[grpno]);
	numsortkeys = add_sort_column(tle->resno, grpcl->sortop,
								  numsortkeys, sortColIdx, sortOperators);
	grpno++;

	Assert(numsortkeys == 1);/*YAS We chech that we are using only 1 column*/
...
/* CHANGED BY RUBY*/
Agg *
make_agg_SGB_DIST(PlannerInfo *root, List *tlist, List *qual,
		 AggStrategy aggstrategy,
		 int numGroupCols, AttrNumber *grpColIdx,
		 long numGroups, int numAggs,
		 Plan *lefttree, int distAll, int overlap, int distAny, int metric)
{
	...
	node->max_group_diam = -1; /*CHANGED BY YASIN*/
	node->max_elem_sep = -1; /*CHANGED BY YASIN*/
	node->distanceall = distAll; /*CHANGED BY RUBY*/
	node->overlap = overlap;
	node->distanceany=distAny;
	node->metric=metric;
	...
	plan->targetlist = tlist;
	plan->lefttree = lefttree;
	plan->righttree = NULL; /*YASIN CAMBIAR AQUI!!!*/
		 ...
		 Plan *lefttree)
{
	...
	node->max_group_diam = -1; /*CHANGED BY YASIN*/
	node->max_elem_sep = -1; /*CHANGED BY YASIN*/
	node->distanceall = -1; /*CHANGED BY RUBY*/
	node->distanceany = -1;
...
/*CHANGED BY YASIN*/
Agg *
make_agg_SGB(PlannerInfo *root, List *tlist, List *qual,
		 AggStrategy aggstrategy,
		 int numGroupCols, AttrNumber *grpColIdx,
		 long numGroups, int numAggs,
		 Plan *lefttree,
		 Plan *righttree, /*This is the SGB_around query*/
		 List *subquery_around_rtable,
		 int max_group_diam, /*This is the maximum group diameter parameter*/
		 int max_elem_sep)   /*This is the maximum element separation parameter*/
{
	Agg		   *node = makeNode(Agg);
	Plan	   *plan = &node->plan;
	Path		agg_path;		/* dummy for result of cost_agg */
	...
	node->grpColIdx = grpColIdx;
	node->numGroups = numGroups;
	node->subquery_around_rtable = subquery_around_rtable;
	node->max_group_diam = max_group_diam;
	node->max_elem_sep = max_elem_sep;
	node->distanceall = -1;
	...
	plan->targetlist = tlist;
	plan->lefttree = lefttree;
	plan->righttree = righttree; /*CHAGED BY YASIN*/
...
/*CHANGED BY YASIN*/
Group *
make_group_around(PlannerInfo *root,
		   List *tlist,
		   List *qual,
		   int numGroupCols,
		   AttrNumber *grpColIdx,
		   ...
		   int max_elem_sep)
{
	...
	plan->targetlist = tlist;
	plan->lefttree = lefttree;
	plan->righttree = righttree;/*CHANGED BY YASIN*/
