			  ...
			  double *num_groups)
{
	...
	*num_groups = 1;			/* default result */
	 ...
	 */
	/*printf("root antes de make_on_rel\n");pprint(root);*/	 
	final_rel = make_one_rel(root, joinlist);
	/*printf("root despues de make_on_rel\n"); pprint(root); pprint(final_rel);*/
	...
	if (parse->groupClause)
	{
		...
		groupExprs = get_sortgrouplist_exprs(parse->groupClause,/*YASIN, THIS PUTS TOGETHER ALL THE EXPRESIONS THAT USE GROUP EXPRESIONS*/
											 parse->targetList);
