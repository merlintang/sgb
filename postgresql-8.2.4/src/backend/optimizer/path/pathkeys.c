...
/*
 * pathkeys_contained_in
 *	  compare_pathkeys: we just want to know
 *	  if keys2[0] is sorted as keys1[0].
 */
bool
pathkeys_contained_in_sgb(List *keys1, List *keys2)
{
	ListCell   *key1,
			   *key2;
	List	   *subkey1;
	...
	key2 = list_head(keys2);

	if (key1 == NULL && key2 == NULL) /*path are equal (both null)*/
		return true;
    else if (key1==NULL || key2==NULL) /*Only one is null*/
    	return false;
	/*At this point key1  and key2 are not null*/	
	subkey1 = (List *) lfirst(key1);
	subkey2 = (List *) lfirst(key2);
