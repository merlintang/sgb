...
//pprint (parsetree);
		...
		querytree_list = pg_analyze_and_rewrite(parsetree, query_string,
												NULL, 0);
//printf("QUERY TREE ANTES DE PLANNING:\n");
//pprint (querytree_list);
	...
	ColumnDef *d = makeNode(ColumnDef);
	d->colname = "s";

	TypeName * t = makeNode(TypeName);
    List *l = 	NIL;
	Value *v = makeString("seg");
	...
	relOid = DefineRelation(stmt, RELKIND_RELATION);*/
...
/*		IndexStmt  *stmt = makeNode(IndexStmt);
		stmt->idxname = "test0123";
		stmt->accessMethod = "gist";

		IndexElem *e = makeNode(IndexElem);
		e->name = "s";
