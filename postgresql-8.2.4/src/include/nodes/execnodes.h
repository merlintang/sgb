...
typedef struct EState
{
	...
	/*CHANGED BY YASIN
	List	   *es_range_table_around; List of RangeTableEntrys of the right plan in an agg/group node*/
...
typedef struct ScanState
{
	...
	TupleTableSlot *ss_ScanTupleSlot;/* pointer to slot in tuple table holding scan tuple*/
} ScanState;
...
typedef struct MergeJoinState
{
	...
	/*Changed by Yasin SJ*/
	int			mj_SimjoinType;/* 0 = No SJ, 1=Eps-Join, 2=Join-Around*/
	TupleTableSlot *mj_PreviousInnerTupleSlot;/*Slot for previous inner*/
	TupleTableSlot *mj_NextInnerTupleSlot;/*Slot for next inner*/
	/*Only for Join-Around*/
	TupleTableSlot *mj_InnerCopyTupleSlot;/*Slot that points to an independent copy of inner*/
	TupleTableSlot *mj_FirstMarkedSlot;/*Stores the first marked tuple. In Join around, regular marked starts in next inner*/
...
typedef struct AggHashEntryData
{
	...
	/*CHANGED BY YASIN*/
	char flagEnabled;/*0=initial value (Not enabled), 1=Analized(Not enabled), 2=Enabled 3=Data Empty but the bounds was not*/

	/*Changed By Mingjie*/
	void	   *tuplestorestate;	/* private state of tuplestore.c to store the member in this group*/
	void 	   *tupleInmemolist;    /* put the tuple from the tuplestorestate */
	void 	   *convexhull;
	int pivot1Slot[5];		/* group pivot p1 */
	int pivot2Slot[5];		/* group pivot p2 */
    bool ismodified;     /*to indicate this entry have been modified*/
	/* per-aggregate transition status array - must be last! */
	AggStatePerGroupData pergroup[1];	/* VARIABLE LENGTH ARRAY */
} AggHashEntryData;				/* VARIABLE LENGTH STRUCT */

/*modifed by Mingjie*/
typedef struct AggHashTuple_M
{
	int inputslots[5];
	int pivot1[5];
	int pivot2[5];
...
typedef struct AggState
{
	...
	AggStatePerAgg peragg;		/* per-Aggref information */ /*INITIALIZED AT BEGINNING OF EACH GROUP AND UPDATED WITH EACH TUPLE. HAS ALSO INFO THAT REMAINS CONSTANT*/
	MemoryContext aggcontext;	/* memory context for LONG-LIVED data */
	ExprContext *tmpcontext;	/* econtext for INPUT expressions */
	bool		agg_done;		/* indicates completion of Agg scan */
	/* these fields are used in AGG_PLAIN and AGG_SORTED modes: */
	AggStatePerGroup pergroup;	/* per-Aggref-per-group working state */ /*INITIALIZED AT THE BEGINNING OF EACH GROUP AND UPDATED WITH EACH TUPLE transValue*/
	HeapTuple	grp_firstTuple; /* copy of FIRST TUPLE OF CURRENT GROUP*/

	/*CHANGED BY YASIN*/
	HeapTuple	grp_Pivot1HeapTuple; /* copy of FIRST PIVOT TUPLE OF CURRENT GROUP*/
	HeapTuple	grp_Pivot2HeapTuple; /* copy of SECOND PIVOT TUPLE OF CURRENT GROUP*/
	FmgrInfo   *ltfunctions;	/* per-grouping-field less than fns */
	FmgrInfo   *minusfunctions;	/* per-grouping-field minus fns */
	TupleTableSlot *pivot1Slot;	/* slot for loading pivot1*/
	TupleTableSlot *pivot2Slot;	/* slot for loading pivot2*/
	TupleTableSlot *modifiedSlot; 	/* slot for updated tuples used in hash grouping*/
	void	   *overlaptuplestorestate1;	/* private state of tuplestore.c to store the candiddate list*/
	void	   *overlaptuplestorestate2;	/* private state of tuplestore.c to store the candiddate list*/
	...
	AggHashTupleM *hasharray;	/*by MINGJIE for storing the entry for mapping this to r tree*/
	int indexhasharray; 		/*by MINGJIE for the index for the hash array*/
	int numbertuples;			/*by MINGJIE for the number of tuples*/
	HRTREEROOT rtreeroot; 		/*rtree root node for one aggstate by Mingjie*/
	DisJointSets *dsjset;		/*by MINGJIE for SGBANY DISJOINT SET*/
} AggState;
